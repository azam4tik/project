#include "test.h"

#define ERR_MAT 1
#define ERR_CAL 2
#define MAX_DOUBLE 1.79769e+308
#define MIN_DOUBLE 2.22507e-308

void s21_init_matrix(double number, matrix_t *A) {
  for (int x = 0; x < A->rows; x += 1) {
    for (int y = 0; y < A->columns; number += 1.0, y += 1)
      A->matrix[x][y] = number;
  }
}

START_TEST(s21_create_matrix_01) {
  int res = 0;
  matrix_t A = {0};

  res = s21_create_matrix(1, 1, &A);
  ck_assert_int_eq(res, OK);
  s21_remove_matrix(&A);
}
END_TEST

START_TEST(s21_create_matrix_02) {
  int res = 0;
  matrix_t A = {0};

  res = s21_create_matrix(2, 2, &A);
  ck_assert_int_eq(res, OK);
  s21_remove_matrix(&A);
}
END_TEST

START_TEST(s21_create_matrix_03) {
  int res = 0;
  matrix_t A = {0};

  res = s21_create_matrix(3, 2, &A);
  ck_assert_int_eq(res, OK);
  s21_remove_matrix(&A);
}
END_TEST

START_TEST(s21_create_matrix_04) {
  int res = 0;
  matrix_t A = {0};

  res = s21_create_matrix(2, 4, &A);
  ck_assert_int_eq(res, OK);
  s21_remove_matrix(&A);
}
END_TEST

START_TEST(s21_create_matrix_05) {
  int res = 0;
  matrix_t A = {0};

  res = s21_create_matrix(10, 15, &A);
  ck_assert_int_eq(res, OK);
  s21_remove_matrix(&A);
}
END_TEST

START_TEST(s21_create_matrix_06) {
  int res = 0;
  matrix_t A = {0};

  res = s21_create_matrix(0, 0, &A);
  ck_assert_int_eq(res, ERR_MAT);
}
END_TEST

START_TEST(s21_create_matrix_07) {
  int res = 0;
  matrix_t A = {0};

  res = s21_create_matrix(1, 0, &A);
  ck_assert_int_eq(res, ERR_MAT);
}
END_TEST

START_TEST(s21_create_matrix_08) {
  int res = 0;
  matrix_t A = {0};

  res = s21_create_matrix(0, 1, &A);
  ck_assert_int_eq(res, ERR_MAT);
}
END_TEST

START_TEST(s21_create_matrix_09) {
  int res = 0;
  matrix_t A = {0};

  res = s21_create_matrix(-1, 1, &A);
  ck_assert_int_eq(res, ERR_MAT);
}
END_TEST

START_TEST(s21_remove_matrix_01) {
  int res = 0;
  matrix_t A = {0};

  res = s21_create_matrix(2, 2, &A);
  s21_remove_matrix(&A);
  ck_assert_int_eq(res, OK);
}
END_TEST

START_TEST(s21_remove_matrix_02) {
  int res = 0;
  matrix_t A = {0};

  res = s21_create_matrix(1, 1, &A);
  s21_remove_matrix(&A);
  ck_assert_int_eq(res, OK);
}
END_TEST

START_TEST(s21_remove_matrix_03) {
  int res = 0;
  matrix_t A = {0};

  res = s21_create_matrix(3, 2, &A);
  s21_remove_matrix(&A);
  ck_assert_int_eq(res, OK);
}
END_TEST

START_TEST(s21_remove_matrix_04) {
  int res = 0;
  matrix_t A = {0};

  res = s21_create_matrix(3, 4, &A);
  s21_remove_matrix(&A);
  ck_assert_int_eq(res, OK);
}
END_TEST

START_TEST(s21_remove_matrix_05) {
  int res = 0;
  matrix_t A = {0};

  res = s21_create_matrix(3, 2, &A);
  s21_remove_matrix(&A);
  ck_assert_int_eq(res, OK);
}
END_TEST

START_TEST(s21_eq_matrix_01) {
  int res = 0;
  matrix_t A = {0};
  matrix_t B = {0};

  int resA = s21_create_matrix(3, 2, &A);
  int resB = s21_create_matrix(3, 2, &B);
  if (resA == 0 && resB == 0) {
    s21_init_matrix(1.0, &A);
    s21_init_matrix(1.0, &B);

    res = s21_eq_matrix(&A, &B);
    ck_assert_int_eq(res, SUCCESS);
  }
  s21_remove_matrix(&A);
  s21_remove_matrix(&B);
}
END_TEST

START_TEST(s21_eq_matrix_06) {
  int res = 0;
  matrix_t A = {0};
  matrix_t B = {0};

  int resA = s21_create_matrix(10, 10, &A);
  int resB = s21_create_matrix(10, 10, &B);
  if (resA == 0 && resB == 0) {
    s21_init_matrix(1.0, &A);
    s21_init_matrix(1.0, &B);

    res = s21_eq_matrix(&A, &B);
    ck_assert_int_eq(res, SUCCESS);
  }
  s21_remove_matrix(&A);
  s21_remove_matrix(&B);
}
END_TEST

START_TEST(s21_sum_matrix_01) {
  int res = 0;
  matrix_t A = {0};
  matrix_t B = {0};
  matrix_t Z = {0};
  matrix_t X = {0};

  int resA = s21_create_matrix(4, 4, &A);
  int resB = s21_create_matrix(4, 4, &B);
  int resC = s21_create_matrix(4, 4, &X);
  if (resA == 0 && resB == 0 && resC == 0) {
    s21_init_matrix(1.0, &A);
    s21_init_matrix(1.0, &B);

    s21_sum_matrix(&A, &B, &Z);

    X.matrix[0][0] = 2.0;
    X.matrix[0][1] = 4.0;
    X.matrix[0][2] = 6.0;
    X.matrix[0][3] = 8.0;
    X.matrix[1][0] = 10.0;
    X.matrix[1][1] = 12.0;
    X.matrix[1][2] = 14.0;
    X.matrix[1][3] = 16.0;
    X.matrix[2][0] = 18.0;
    X.matrix[2][1] = 20.0;
    X.matrix[2][2] = 22.0;
    X.matrix[2][3] = 24.0;
    X.matrix[3][0] = 26.0;
    X.matrix[3][1] = 28.0;
    X.matrix[3][2] = 30.0;
    X.matrix[3][3] = 32.0;

    res = s21_eq_matrix(&X, &Z);
    ck_assert_int_eq(res, SUCCESS);
  }
  s21_remove_matrix(&A);
  s21_remove_matrix(&B);
  s21_remove_matrix(&X);
  s21_remove_matrix(&Z);
}
END_TEST

START_TEST(s21_sum_matrix_02) {
  int res = 0;
  matrix_t A = {0};
  matrix_t B = {0};
  matrix_t Z = {0};
  matrix_t X = {0};

  int resA = s21_create_matrix(4, 4, &A);
  int resB = s21_create_matrix(4, 4, &B);
  int resC = s21_create_matrix(4, 4, &X);
  if (resA == 0 && resB == 0 && resC == 0) {
    s21_init_matrix(0.0, &A);
    s21_init_matrix(2.0, &B);

    s21_sum_matrix(&A, &B, &Z);

    X.matrix[0][0] = 2.0;
    X.matrix[0][1] = 4.0;
    X.matrix[0][2] = 6.0;
    X.matrix[0][3] = 8.0;
    X.matrix[1][0] = 10.0;
    X.matrix[1][1] = 12.0;
    X.matrix[1][2] = 14.0;
    X.matrix[1][3] = 16.0;
    X.matrix[2][0] = 18.0;
    X.matrix[2][1] = 20.0;
    X.matrix[2][2] = 22.0;
    X.matrix[2][3] = 24.0;
    X.matrix[3][0] = 26.0;
    X.matrix[3][1] = 28.0;
    X.matrix[3][2] = 30.0;
    X.matrix[3][3] = 32.0;

    res = s21_eq_matrix(&X, &Z);
    ck_assert_int_eq(res, SUCCESS);
  }
  s21_remove_matrix(&A);
  s21_remove_matrix(&B);
  s21_remove_matrix(&X);
  s21_remove_matrix(&Z);
}
END_TEST

START_TEST(s21_sum_matrix_03) {
  int res = 0;
  matrix_t A = {0};
  matrix_t B = {0};
  matrix_t Z = {0};
  matrix_t X = {0};

  int resA = s21_create_matrix(4, 4, &A);
  int resB = s21_create_matrix(4, 4, &B);
  int resC = s21_create_matrix(4, 4, &X);
  if (resA == 0 && resB == 0 && resC == 0) {
    s21_init_matrix(-3.0, &A);
    s21_init_matrix(5.0, &B);
    s21_sum_matrix(&A, &B, &Z);

    X.matrix[0][0] = 2.0;
    X.matrix[0][1] = 4.0;
    X.matrix[0][2] = 6.0;
    X.matrix[0][3] = 8.0;
    X.matrix[1][0] = 10.0;
    X.matrix[1][1] = 12.0;
    X.matrix[1][2] = 14.0;
    X.matrix[1][3] = 16.0;
    X.matrix[2][0] = 18.0;
    X.matrix[2][1] = 20.0;
    X.matrix[2][2] = 22.0;
    X.matrix[2][3] = 24.0;
    X.matrix[3][0] = 26.0;
    X.matrix[3][1] = 28.0;
    X.matrix[3][2] = 30.0;
    X.matrix[3][3] = 32.0;

    res = s21_eq_matrix(&X, &Z);
    ck_assert_int_eq(res, SUCCESS);
  }
  s21_remove_matrix(&A);
  s21_remove_matrix(&B);
  s21_remove_matrix(&X);
  s21_remove_matrix(&Z);
}
END_TEST

START_TEST(s21_sum_matrix_04) {
  int res = 0;
  matrix_t A = {0};
  matrix_t B = {0};
  matrix_t Z = {0};
  matrix_t X = {0};

  int resA = s21_create_matrix(4, 4, &A);
  int resB = s21_create_matrix(4, 4, &B);
  int resC = s21_create_matrix(4, 4, &X);
  if (resA == 0 && resB == 0 && resC == 0) {
    s21_init_matrix(5.0, &A);
    s21_init_matrix(-3.0, &B);

    s21_sum_matrix(&A, &B, &Z);

    X.matrix[0][0] = 2.0;
    X.matrix[0][1] = 4.0;
    X.matrix[0][2] = 6.0;
    X.matrix[0][3] = 8.0;
    X.matrix[1][0] = 10.0;
    X.matrix[1][1] = 12.0;
    X.matrix[1][2] = 14.0;
    X.matrix[1][3] = 16.0;
    X.matrix[2][0] = 18.0;
    X.matrix[2][1] = 20.0;
    X.matrix[2][2] = 22.0;
    X.matrix[2][3] = 24.0;
    X.matrix[3][0] = 26.0;
    X.matrix[3][1] = 28.0;
    X.matrix[3][2] = 30.0;
    X.matrix[3][3] = 32.0;

    res = s21_eq_matrix(&X, &Z);
    ck_assert_int_eq(res, SUCCESS);
  }
  s21_remove_matrix(&A);
  s21_remove_matrix(&B);
  s21_remove_matrix(&X);
  s21_remove_matrix(&Z);
}
END_TEST

START_TEST(s21_sum_matrix_05) {
  int res = 0;
  matrix_t A = {0};
  matrix_t B = {0};
  matrix_t Z = {0};
  matrix_t X = {0};
  int resA = s21_create_matrix(4, 2, &A);
  int resB = s21_create_matrix(4, 2, &B);
  int resC = s21_create_matrix(4, 2, &X);
  if (resA == 0 && resB == 0 && resC == 0) {
    s21_init_matrix(1.0, &A);
    s21_init_matrix(1.0, &B);

    s21_sum_matrix(&A, &B, &Z);

    X.matrix[0][0] = 2.0;
    X.matrix[0][1] = 4.0;
    X.matrix[1][0] = 6.0;
    X.matrix[1][1] = 8.0;
    X.matrix[2][0] = 10.0;
    X.matrix[2][1] = 12.0;
    X.matrix[3][0] = 14.0;
    X.matrix[3][1] = 16.0;

    res = s21_eq_matrix(&X, &Z);
    ck_assert_int_eq(res, SUCCESS);
  }
  s21_remove_matrix(&A);
  s21_remove_matrix(&B);
  s21_remove_matrix(&X);
  s21_remove_matrix(&Z);
}
END_TEST

START_TEST(s21_sum_matrix_06) {
  int res = 0;
  matrix_t A = {0};
  matrix_t B = {0};
  matrix_t Z = {0};
  matrix_t X = {0};
  int resA = s21_create_matrix(4, 2, &A);
  int resB = s21_create_matrix(4, 2, &B);
  int resC = s21_create_matrix(4, 2, &X);
  if (resA == 0 && resB == 0 && resC == 0) {
    s21_init_matrix(-1.0, &A);
    s21_init_matrix(3.0, &B);

    s21_sum_matrix(&A, &B, &Z);

    X.matrix[0][0] = 2.0;
    X.matrix[0][1] = 4.0;
    X.matrix[1][0] = 6.0;
    X.matrix[1][1] = 8.0;
    X.matrix[2][0] = 10.0;
    X.matrix[2][1] = 12.0;
    X.matrix[3][0] = 14.0;
    X.matrix[3][1] = 16.0;

    res = s21_eq_matrix(&X, &Z);
    ck_assert_int_eq(res, SUCCESS);
  }
  s21_remove_matrix(&A);
  s21_remove_matrix(&B);
  s21_remove_matrix(&X);
  s21_remove_matrix(&Z);
}
END_TEST

START_TEST(s21_sum_matrix_07) {
  int res = 0;
  matrix_t A = {0};
  matrix_t B = {0};
  matrix_t Z = {0};
  matrix_t X = {0};
  int resA = s21_create_matrix(4, 2, &A);
  int resB = s21_create_matrix(4, 2, &B);
  int resC = s21_create_matrix(4, 2, &X);
  if (resA == 0 && resB == 0 && resC == 0) {
    s21_init_matrix(4.0, &A);
    s21_init_matrix(-2.0, &B);

    s21_sum_matrix(&A, &B, &Z);

    X.matrix[0][0] = 2.0;
    X.matrix[0][1] = 4.0;
    X.matrix[1][0] = 6.0;
    X.matrix[1][1] = 8.0;
    X.matrix[2][0] = 10.0;
    X.matrix[2][1] = 12.0;
    X.matrix[3][0] = 14.0;
    X.matrix[3][1] = 16.0;

    res = s21_eq_matrix(&X, &Z);
    ck_assert_int_eq(res, SUCCESS);
  }
  s21_remove_matrix(&A);
  s21_remove_matrix(&B);
  s21_remove_matrix(&X);
  s21_remove_matrix(&Z);
}
END_TEST

START_TEST(s21_sum_matrix_08) {
  int res = 0;
  matrix_t A = {0};
  matrix_t B = {0};
  matrix_t Z = {0};

  int resA = s21_create_matrix(4, 2, &A);
  int resB = s21_create_matrix(4, 3, &B);
  if (resA == 0 && resB == 0) {
    res = s21_sum_matrix(&A, &B, &Z);
    ck_assert_int_eq(res, ERR_CAL);
  }
  s21_remove_matrix(&A);
  s21_remove_matrix(&B);
  s21_remove_matrix(&Z);
}
END_TEST

START_TEST(s21_sub_matrix_01) {
  int res = 0;
  matrix_t A = {0};
  matrix_t B = {0};
  matrix_t Z = {0};
  matrix_t X = {0};

  int resA = s21_create_matrix(4, 4, &A);
  int resB = s21_create_matrix(4, 4, &X);
  int resC = s21_create_matrix(4, 4, &B);
  if (resA == 0 && resB == 0 && resC == 0) {
    s21_init_matrix(1.0, &A);
    s21_init_matrix(1.0, &B);

    s21_sub_matrix(&A, &B, &Z);

    X.matrix[0][0] = 0.0;
    X.matrix[0][1] = 0.0;
    X.matrix[0][2] = 0.0;
    X.matrix[0][3] = 0.0;
    X.matrix[1][0] = 0.0;
    X.matrix[1][1] = 0.0;
    X.matrix[1][2] = 0.0;
    X.matrix[1][3] = 0.0;
    X.matrix[2][0] = 0.0;
    X.matrix[2][1] = 0.0;
    X.matrix[2][2] = 0.0;
    X.matrix[2][3] = 0.0;
    X.matrix[3][0] = 0.0;
    X.matrix[3][1] = 0.0;
    X.matrix[3][2] = 0.0;
    X.matrix[3][3] = 0.0;

    res = s21_eq_matrix(&X, &Z);
    ck_assert_int_eq(res, SUCCESS);
  }
  s21_remove_matrix(&A);
  s21_remove_matrix(&B);
  s21_remove_matrix(&X);
  s21_remove_matrix(&Z);
}
END_TEST

START_TEST(s21_sub_matrix_02) {
  int res = 0;
  matrix_t A = {0};
  matrix_t B = {0};
  matrix_t Z = {0};
  matrix_t X = {0};
  int resA = s21_create_matrix(4, 4, &A);
  int resB = s21_create_matrix(4, 4, &X);
  int resC = s21_create_matrix(4, 4, &B);
  if (resA == 0 && resB == 0 && resC == 0) {
    s21_init_matrix(0.0, &A);
    s21_init_matrix(2.0, &B);
    s21_sub_matrix(&A, &B, &Z);

    X.matrix[0][0] = -2.0;
    X.matrix[0][1] = -2.0;
    X.matrix[0][2] = -2.0;
    X.matrix[0][3] = -2.0;
    X.matrix[1][0] = -2.0;
    X.matrix[1][1] = -2.0;
    X.matrix[1][2] = -2.0;
    X.matrix[1][3] = -2.0;
    X.matrix[2][0] = -2.0;
    X.matrix[2][1] = -2.0;
    X.matrix[2][2] = -2.0;
    X.matrix[2][3] = -2.0;
    X.matrix[3][0] = -2.0;
    X.matrix[3][1] = -2.0;
    X.matrix[3][2] = -2.0;
    X.matrix[3][3] = -2.0;

    res = s21_eq_matrix(&X, &Z);
    ck_assert_int_eq(res, SUCCESS);
  }
  s21_remove_matrix(&A);
  s21_remove_matrix(&B);
  s21_remove_matrix(&X);
  s21_remove_matrix(&Z);
}
END_TEST

START_TEST(s21_sub_matrix_03) {
  int res = 0;
  matrix_t A = {0};
  matrix_t B = {0};
  matrix_t Z = {0};
  matrix_t X = {0};
  int resA = s21_create_matrix(4, 4, &A);
  int resB = s21_create_matrix(4, 4, &X);
  int resC = s21_create_matrix(4, 4, &B);
  if (resA == 0 && resB == 0 && resC == 0) {
    s21_init_matrix(-3.0, &A);
    s21_init_matrix(5.0, &B);
    s21_sub_matrix(&A, &B, &Z);

    X.matrix[0][0] = -8.0;
    X.matrix[0][1] = -8.0;
    X.matrix[0][2] = -8.0;
    X.matrix[0][3] = -8.0;
    X.matrix[1][0] = -8.0;
    X.matrix[1][1] = -8.0;
    X.matrix[1][2] = -8.0;
    X.matrix[1][3] = -8.0;
    X.matrix[2][0] = -8.0;
    X.matrix[2][1] = -8.0;
    X.matrix[2][2] = -8.0;
    X.matrix[2][3] = -8.0;
    X.matrix[3][0] = -8.0;
    X.matrix[3][1] = -8.0;
    X.matrix[3][2] = -8.0;
    X.matrix[3][3] = -8.0;

    res = s21_eq_matrix(&X, &Z);
    ck_assert_int_eq(res, SUCCESS);
  }
  s21_remove_matrix(&A);
  s21_remove_matrix(&B);
  s21_remove_matrix(&X);
  s21_remove_matrix(&Z);
}
END_TEST

START_TEST(s21_sub_matrix_04) {
  int res = 0;
  matrix_t A = {0};
  matrix_t B = {0};
  matrix_t Z = {0};
  matrix_t X = {0};
  int resA = s21_create_matrix(4, 4, &A);
  int resB = s21_create_matrix(4, 4, &X);
  int resC = s21_create_matrix(4, 4, &B);
  if (resA == 0 && resB == 0 && resC == 0) {
    s21_init_matrix(5.0, &A);
    s21_init_matrix(-3.0, &B);
    s21_sub_matrix(&A, &B, &Z);

    X.matrix[0][0] = 8.0;
    X.matrix[0][1] = 8.0;
    X.matrix[0][2] = 8.0;
    X.matrix[0][3] = 8.0;
    X.matrix[1][0] = 8.0;
    X.matrix[1][1] = 8.0;
    X.matrix[1][2] = 8.0;
    X.matrix[1][3] = 8.0;
    X.matrix[2][0] = 8.0;
    X.matrix[2][1] = 8.0;
    X.matrix[2][2] = 8.0;
    X.matrix[2][3] = 8.0;
    X.matrix[3][0] = 8.0;
    X.matrix[3][1] = 8.0;
    X.matrix[3][2] = 8.0;
    X.matrix[3][3] = 8.0;

    res = s21_eq_matrix(&X, &Z);
    ck_assert_int_eq(res, SUCCESS);
  }
  s21_remove_matrix(&A);
  s21_remove_matrix(&B);
  s21_remove_matrix(&X);
  s21_remove_matrix(&Z);
}
END_TEST

START_TEST(s21_sub_matrix_05) {
  int res = 0;
  matrix_t A = {0};
  matrix_t B = {0};
  matrix_t Z = {0};
  matrix_t X = {0};
  int resA = s21_create_matrix(4, 2, &A);
  int resB = s21_create_matrix(4, 2, &X);
  int resC = s21_create_matrix(4, 2, &B);
  if (resA == 0 && resB == 0 && resC == 0) {
    s21_init_matrix(1.0, &A);
    s21_init_matrix(1.0, &B);
    s21_sub_matrix(&A, &B, &Z);

    X.matrix[0][0] = 0.0;
    X.matrix[0][1] = 0.0;
    X.matrix[1][0] = 0.0;
    X.matrix[1][1] = 0.0;
    X.matrix[2][0] = 0.0;
    X.matrix[2][1] = 0.0;
    X.matrix[3][0] = 0.0;
    X.matrix[3][1] = 0.0;

    res = s21_eq_matrix(&X, &Z);
    ck_assert_int_eq(res, SUCCESS);
  }
  s21_remove_matrix(&A);
  s21_remove_matrix(&B);
  s21_remove_matrix(&X);
  s21_remove_matrix(&Z);
}
END_TEST

START_TEST(s21_sub_matrix_06) {
  int res = 0;
  matrix_t A = {0};
  matrix_t B = {0};
  matrix_t Z = {0};
  matrix_t X = {0};
  int resA = s21_create_matrix(4, 2, &A);
  int resB = s21_create_matrix(4, 2, &X);
  int resC = s21_create_matrix(4, 2, &B);
  if (resA == 0 && resB == 0 && resC == 0) {
    s21_init_matrix(-1.0, &A);
    s21_init_matrix(3.0, &B);
    s21_sub_matrix(&A, &B, &Z);

    X.matrix[0][0] = -4.0;
    X.matrix[0][1] = -4.0;
    X.matrix[1][0] = -4.0;
    X.matrix[1][1] = -4.0;
    X.matrix[2][0] = -4.0;
    X.matrix[2][1] = -4.0;
    X.matrix[3][0] = -4.0;
    X.matrix[3][1] = -4.0;

    res = s21_eq_matrix(&X, &Z);
    ck_assert_int_eq(res, SUCCESS);
  }
  s21_remove_matrix(&A);
  s21_remove_matrix(&B);
  s21_remove_matrix(&X);
  s21_remove_matrix(&Z);
}
END_TEST

START_TEST(s21_sub_matrix_07) {
  int res = 0;
  matrix_t A = {0};
  matrix_t B = {0};
  matrix_t Z = {0};
  matrix_t X = {0};
  int resA = s21_create_matrix(4, 2, &A);
  int resB = s21_create_matrix(4, 2, &X);
  int resC = s21_create_matrix(4, 2, &B);
  if (resA == 0 && resB == 0 && resC == 0) {
    s21_init_matrix(4.0, &A);
    s21_init_matrix(-2.0, &B);
    s21_sub_matrix(&A, &B, &Z);

    X.matrix[0][0] = 6.0;
    X.matrix[0][1] = 6.0;
    X.matrix[1][0] = 6.0;
    X.matrix[1][1] = 6.0;
    X.matrix[2][0] = 6.0;
    X.matrix[2][1] = 6.0;
    X.matrix[3][0] = 6.0;
    X.matrix[3][1] = 6.0;

    res = s21_eq_matrix(&X, &Z);
    ck_assert_int_eq(res, SUCCESS);
  }
  s21_remove_matrix(&A);
  s21_remove_matrix(&B);
  s21_remove_matrix(&X);
  s21_remove_matrix(&Z);
}
END_TEST

START_TEST(s21_sub_matrix_08) {
  int res = 0;
  matrix_t A = {0};
  matrix_t B = {0};
  matrix_t Z = {0};
  int resA = s21_create_matrix(4, 2, &A);
  int resB = s21_create_matrix(4, 3, &B);
  if (resA == 0 && resB == 0) {
    res = s21_sub_matrix(&A, &B, &Z);
    ck_assert_int_eq(res, ERR_CAL);
  }
  s21_remove_matrix(&A);
  s21_remove_matrix(&B);
  s21_remove_matrix(&Z);
}
END_TEST

START_TEST(s21_mult_number_01) {
  int res = 0;
  double number = 3.0;
  matrix_t A = {0};
  matrix_t Z = {0};
  matrix_t X = {0};

  int resA = s21_create_matrix(4, 4, &A);
  int resB = s21_create_matrix(4, 4, &X);
  if (resA == 0 && resB == 0) {
    s21_init_matrix(1.0, &A);
    s21_mult_number(&A, number, &Z);

    X.matrix[0][0] = 3.0;
    X.matrix[0][1] = 6.0;
    X.matrix[0][2] = 9.0;
    X.matrix[0][3] = 12.0;
    X.matrix[1][0] = 15.0;
    X.matrix[1][1] = 18.0;
    X.matrix[1][2] = 21.0;
    X.matrix[1][3] = 24.0;
    X.matrix[2][0] = 27.0;
    X.matrix[2][1] = 30.0;
    X.matrix[2][2] = 33.0;
    X.matrix[2][3] = 36.0;
    X.matrix[3][0] = 39.0;
    X.matrix[3][1] = 42.0;
    X.matrix[3][2] = 45.0;
    X.matrix[3][3] = 48.0;

    res = s21_eq_matrix(&X, &Z);
    ck_assert_int_eq(res, SUCCESS);
  }
  s21_remove_matrix(&A);
  s21_remove_matrix(&X);
  s21_remove_matrix(&Z);
}
END_TEST

START_TEST(s21_mult_number_02) {
  int res = 0;
  double number = 5.0;
  matrix_t A = {0};
  matrix_t Z = {0};
  matrix_t X = {0};
  int resA = s21_create_matrix(4, 4, &A);
  int resB = s21_create_matrix(4, 4, &X);
  if (resA == 0 && resB == 0) {
    s21_init_matrix(1.0, &A);
    s21_mult_number(&A, number, &Z);

    X.matrix[0][0] = 5.0;
    X.matrix[0][1] = 10.0;
    X.matrix[0][2] = 15.0;
    X.matrix[0][3] = 20.0;
    X.matrix[1][0] = 25.0;
    X.matrix[1][1] = 30.0;
    X.matrix[1][2] = 35.0;
    X.matrix[1][3] = 40.0;
    X.matrix[2][0] = 45.0;
    X.matrix[2][1] = 50.0;
    X.matrix[2][2] = 55.0;
    X.matrix[2][3] = 60.0;
    X.matrix[3][0] = 65.0;
    X.matrix[3][1] = 70.0;
    X.matrix[3][2] = 75.0;
    X.matrix[3][3] = 80.0;

    res = s21_eq_matrix(&X, &Z);
    ck_assert_int_eq(res, SUCCESS);
  }
  s21_remove_matrix(&A);
  s21_remove_matrix(&X);
  s21_remove_matrix(&Z);
}
END_TEST

START_TEST(s21_mult_number_03) {
  int res = 0;
  double number = 2.0;
  matrix_t A = {0};
  matrix_t Z = {0};
  matrix_t X = {0};
  int resA = s21_create_matrix(4, 2, &A);
  int resB = s21_create_matrix(4, 2, &X);
  if (resA == 0 && resB == 0) {
    s21_init_matrix(2.0, &A);
    s21_mult_number(&A, number, &Z);

    X.matrix[0][0] = 4.0;
    X.matrix[0][1] = 6.0;
    X.matrix[1][0] = 8.0;
    X.matrix[1][1] = 10.0;
    X.matrix[2][0] = 12.0;
    X.matrix[2][1] = 14.0;
    X.matrix[3][0] = 16.0;
    X.matrix[3][1] = 18.0;

    res = s21_eq_matrix(&X, &Z);
    ck_assert_int_eq(res, SUCCESS);
  }
  s21_remove_matrix(&A);
  s21_remove_matrix(&X);
  s21_remove_matrix(&Z);
}
END_TEST

START_TEST(s21_mult_number_04) {
  int res = 0;
  double number = -2.0;
  matrix_t A = {0};
  matrix_t Z = {0};
  matrix_t X = {0};
  int resA = s21_create_matrix(4, 2, &A);
  int resB = s21_create_matrix(4, 2, &X);
  if (resA == 0 && resB == 0) {
    s21_init_matrix(2.0, &A);
    s21_mult_number(&A, number, &Z);

    X.matrix[0][0] = -4.0;
    X.matrix[0][1] = -6.0;
    X.matrix[1][0] = -8.0;
    X.matrix[1][1] = -10.0;
    X.matrix[2][0] = -12.0;
    X.matrix[2][1] = -14.0;
    X.matrix[3][0] = -16.0;
    X.matrix[3][1] = -18.0;

    res = s21_eq_matrix(&X, &Z);
    ck_assert_int_eq(res, SUCCESS);
  }
  s21_remove_matrix(&A);
  s21_remove_matrix(&X);
  s21_remove_matrix(&Z);
}
END_TEST

START_TEST(s21_mult_number_05) {
  int res = 0;
  double number = 2.0;
  matrix_t A = {0};
  matrix_t Z = {0};
  matrix_t X = {0};
  int resA = s21_create_matrix(1, 1, &A);
  int resB = s21_create_matrix(1, 1, &X);
  if (resA == 0 && resB == 0) {
    s21_init_matrix(2.0, &A);
    s21_mult_number(&A, number, &Z);

    X.matrix[0][0] = 4.0;

    res = s21_eq_matrix(&X, &Z);
    ck_assert_int_eq(res, SUCCESS);
  }
  s21_remove_matrix(&A);
  s21_remove_matrix(&X);
  s21_remove_matrix(&Z);
}
END_TEST

START_TEST(s21_mult_number_06) {
  int res = 0;
  double number = 2.0;
  matrix_t A = {0};
  matrix_t Z = {0};

  int resA = s21_create_matrix(0, 0, &A);
  if (resA == 0) {
    res = s21_mult_number(&A, number, &Z);
    ck_assert_int_eq(res, ERR_MAT);
  }
  s21_remove_matrix(&A);
  s21_remove_matrix(&Z);
}
END_TEST

START_TEST(s21_mult_matrix_01) {
  int res = 0;
  matrix_t A = {0};
  matrix_t B = {0};
  matrix_t Z = {0};
  matrix_t X = {0};

  int resA = s21_create_matrix(4, 4, &A);
  int resB = s21_create_matrix(4, 4, &B);
  int resC = s21_create_matrix(4, 4, &X);
  if (resA == 0 && resB == 0 && resC == 0) {
    s21_init_matrix(1.0, &A);
    s21_init_matrix(1.0, &B);

    s21_mult_matrix(&A, &B, &Z);

    X.matrix[0][0] = 90.0;
    X.matrix[0][1] = 100.0;
    X.matrix[0][2] = 110.0;
    X.matrix[0][3] = 120.0;
    X.matrix[1][0] = 202.0;
    X.matrix[1][1] = 228.0;
    X.matrix[1][2] = 254.0;
    X.matrix[1][3] = 280.0;
    X.matrix[2][0] = 314.0;
    X.matrix[2][1] = 356.0;
    X.matrix[2][2] = 398.0;
    X.matrix[2][3] = 440.0;
    X.matrix[3][0] = 426.0;
    X.matrix[3][1] = 484.0;
    X.matrix[3][2] = 542.0;
    X.matrix[3][3] = 600.0;

    res = s21_eq_matrix(&X, &Z);
    ck_assert_int_eq(res, SUCCESS);
  }
  s21_remove_matrix(&A);
  s21_remove_matrix(&B);
  s21_remove_matrix(&X);
  s21_remove_matrix(&Z);
}
END_TEST

START_TEST(s21_mult_matrix_02) {
  int res = 0;
  matrix_t A = {0};
  matrix_t B = {0};
  matrix_t Z = {0};
  matrix_t X = {0};
  int resA = s21_create_matrix(4, 4, &A);
  int resB = s21_create_matrix(4, 4, &B);
  int resC = s21_create_matrix(4, 4, &X);
  if (resA == 0 && resB == 0 && resC == 0) {
    s21_init_matrix(1.0, &A);
    s21_init_matrix(-5.0, &B);
    s21_mult_matrix(&A, &B, &Z);

    X.matrix[0][0] = 30.0;
    X.matrix[0][1] = 40.0;
    X.matrix[0][2] = 50.0;
    X.matrix[0][3] = 60.0;
    X.matrix[1][0] = 46.0;
    X.matrix[1][1] = 72.0;
    X.matrix[1][2] = 98.0;
    X.matrix[1][3] = 124.0;
    X.matrix[2][0] = 62.0;
    X.matrix[2][1] = 104.0;
    X.matrix[2][2] = 146.0;
    X.matrix[2][3] = 188.0;
    X.matrix[3][0] = 78.0;
    X.matrix[3][1] = 136.0;
    X.matrix[3][2] = 194.0;
    X.matrix[3][3] = 252.0;

    res = s21_eq_matrix(&X, &Z);
    ck_assert_int_eq(res, SUCCESS);
  }
  s21_remove_matrix(&A);
  s21_remove_matrix(&B);
  s21_remove_matrix(&X);
  s21_remove_matrix(&Z);
}
END_TEST

START_TEST(s21_mult_matrix_03) {
  int res = 0;
  matrix_t A = {0};
  matrix_t B = {0};
  matrix_t Z = {0};
  matrix_t X = {0};
  int resA = s21_create_matrix(4, 4, &A);
  int resB = s21_create_matrix(4, 4, &B);
  int resC = s21_create_matrix(4, 4, &X);
  if (resA == 0 && resB == 0 && resC == 0) {
    s21_init_matrix(-5.0, &A);
    s21_init_matrix(1.0, &B);
    s21_mult_matrix(&A, &B, &Z);

    X.matrix[0][0] = -78.0;
    X.matrix[0][1] = -92.0;
    X.matrix[0][2] = -106.0;
    X.matrix[0][3] = -120.0;
    X.matrix[1][0] = 34.0;
    X.matrix[1][1] = 36.0;
    X.matrix[1][2] = 38.0;
    X.matrix[1][3] = 40.0;
    X.matrix[2][0] = 146.0;
    X.matrix[2][1] = 164.0;
    X.matrix[2][2] = 182.0;
    X.matrix[2][3] = 200.0;
    X.matrix[3][0] = 258.0;
    X.matrix[3][1] = 292.0;
    X.matrix[3][2] = 326.0;
    X.matrix[3][3] = 360.0;

    res = s21_eq_matrix(&X, &Z);
    ck_assert_int_eq(res, SUCCESS);
  }
  s21_remove_matrix(&A);
  s21_remove_matrix(&B);
  s21_remove_matrix(&X);
  s21_remove_matrix(&Z);
}
END_TEST

START_TEST(s21_mult_matrix_04) {
  int res = 0;
  matrix_t A = {0};
  matrix_t B = {0};
  matrix_t Z = {0};
  matrix_t X = {0};
  int resA = s21_create_matrix(4, 2, &A);
  int resB = s21_create_matrix(2, 4, &B);
  int resC = s21_create_matrix(4, 4, &X);
  if (resA == 0 && resB == 0 && resC == 0) {
    s21_init_matrix(-5.0, &A);
    s21_init_matrix(1.0, &B);
    s21_mult_matrix(&A, &B, &Z);

    X.matrix[0][0] = -25.0;
    X.matrix[0][1] = -34.0;
    X.matrix[0][2] = -43.0;
    X.matrix[0][3] = -52.0;
    X.matrix[1][0] = -13.0;
    X.matrix[1][1] = -18.0;
    X.matrix[1][2] = -23.0;
    X.matrix[1][3] = -28.0;
    X.matrix[2][0] = -1.0;
    X.matrix[2][1] = -2.0;
    X.matrix[2][2] = -3.0;
    X.matrix[2][3] = -4.0;
    X.matrix[3][0] = 11.0;
    X.matrix[3][1] = 14.0;
    X.matrix[3][2] = 17.0;
    X.matrix[3][3] = 20.0;

    res = s21_eq_matrix(&X, &Z);
    ck_assert_int_eq(res, SUCCESS);
  }
  s21_remove_matrix(&A);
  s21_remove_matrix(&B);
  s21_remove_matrix(&X);
  s21_remove_matrix(&Z);
}
END_TEST

START_TEST(s21_mult_matrix_05) {
  int res = 0;
  matrix_t A = {0};
  matrix_t B = {0};
  matrix_t Z = {0};
  matrix_t X = {0};
  int resA = s21_create_matrix(1, 1, &A);
  int resB = s21_create_matrix(1, 1, &B);
  int resC = s21_create_matrix(1, 1, &X);
  if (resA == 0 && resB == 0 && resC == 0) {
    s21_init_matrix(-5.0, &A);
    s21_init_matrix(1.0, &B);

    s21_mult_matrix(&A, &B, &Z);

    X.matrix[0][0] = -5.0;

    res = s21_eq_matrix(&X, &Z);
    ck_assert_int_eq(res, SUCCESS);
  }
  s21_remove_matrix(&A);
  s21_remove_matrix(&B);
  s21_remove_matrix(&X);
  s21_remove_matrix(&Z);
}
END_TEST

START_TEST(s21_mult_matrix_06) {
  int res = 0;
  matrix_t A = {0};
  matrix_t B = {0};
  matrix_t Z = {0};

  res = s21_mult_matrix(&A, &B, &Z);

  s21_remove_matrix(&A);
  s21_remove_matrix(&B);
  s21_remove_matrix(&Z);

  ck_assert_int_eq(res, ERR_MAT);
}
END_TEST

START_TEST(s21_mult_matrix_07) {
  int res = 0;
  matrix_t A = {0};
  matrix_t B = {0};
  matrix_t Z = {0};

  int resA = s21_create_matrix(2, 3, &A);
  int resB = s21_create_matrix(2, 2, &B);
  if (resA == 0 && resB == 0) {
    s21_init_matrix(-5.0, &A);
    s21_init_matrix(1.0, &B);

    res = s21_mult_matrix(&A, &B, &Z);
    ck_assert_int_eq(res, ERR_CAL);
  }
  s21_remove_matrix(&A);
  s21_remove_matrix(&B);
  s21_remove_matrix(&Z);
}
END_TEST

START_TEST(s21_transpose_01) {
  int res = 0;
  matrix_t A = {0};

  res = s21_transpose(&A, &A);
  s21_remove_matrix(&A);
  ck_assert_int_eq(res, ERR_MAT);
}
END_TEST

START_TEST(s21_transpose_03) {
  int res = 0;
  matrix_t A = {0};
  matrix_t T = {0};
  matrix_t B = {0};

  int resA = s21_create_matrix(2, 3, &A);
  int resB = s21_create_matrix(3, 2, &B);
  if (resA == 0 && resB == 0) {
    s21_init_matrix(1.0, &A);
    B.matrix[0][0] = 1.0;
    B.matrix[0][1] = 4.0;
    B.matrix[1][0] = 2.0;
    B.matrix[1][1] = 5.0;
    B.matrix[2][0] = 3.0;
    B.matrix[2][1] = 6.0;

    res = s21_transpose(&A, &T);
    res = s21_eq_matrix(&T, &B);
    ck_assert_int_eq(res, SUCCESS);
  }
  s21_remove_matrix(&A);
  s21_remove_matrix(&T);
  s21_remove_matrix(&B);
}
END_TEST

START_TEST(s21_determinant_01) {
  int res = 0;
  double determ = 0.0;
  matrix_t A = {NULL, 0, 0};

  res = s21_determinant(&A, &determ);
  s21_remove_matrix(&A);
  ck_assert_int_eq(res, ERR_MAT);
}
END_TEST

START_TEST(s21_determinant_02) {
  int res = 0;
  double determ = 0.0;
  matrix_t A = {0};
  int resA = s21_create_matrix(1, 2, &A);
  if (resA == 0) {
    res = s21_determinant(&A, &determ);
    ck_assert_int_eq(res, ERR_CAL);
  }
  s21_remove_matrix(&A);
}
END_TEST

START_TEST(s21_determinant_03) {
  double determ = 0.0;
  matrix_t A = {0};

  int resA = s21_create_matrix(1, 1, &A);
  if (resA == 0) {
    s21_init_matrix(1.0, &A);

    s21_determinant(&A, &determ);
    ck_assert_double_eq(determ, 1.0);
  }
  s21_remove_matrix(&A);
}
END_TEST

START_TEST(s21_determinant_04) {
  double determ = 0.0;
  matrix_t A = {0};

  int resA = s21_create_matrix(2, 2, &A);
  if (resA == 0) {
    A.matrix[0][0] = -677700.0;
    A.matrix[0][1] = 654.0;
    A.matrix[1][0] = 2.0;
    A.matrix[1][1] = -0.00001;

    s21_determinant(&A, &determ);
    ck_assert_double_eq(determ, -1301.223);
  }
  s21_remove_matrix(&A);
}
END_TEST

START_TEST(s21_determinant_05) {
  double determ = 0.0;
  matrix_t A = {0};

  int resA = s21_create_matrix(8, 8, &A);
  if (resA == 0) {
    s21_init_matrix(1.0, &A);

    s21_determinant(&A, &determ);
    ck_assert_double_eq(determ, 0.0);
  }
  s21_remove_matrix(&A);
}
END_TEST

START_TEST(s21_calc_complements_01) {
  int res = 0;
  matrix_t A = {0};
  matrix_t Z = {0};
  matrix_t X = {0};

  int resA = s21_create_matrix(4, 4, &A);
  int resB = s21_create_matrix(4, 4, &X);
  if (resA == 0 && resB == 0) {
    A.matrix[0][0] = 112.0;
    A.matrix[0][1] = 243.0;
    A.matrix[0][2] = 3.0;
    A.matrix[0][3] = -8.0;
    A.matrix[1][0] = 47.0;
    A.matrix[1][1] = 51.0;
    A.matrix[1][2] = -66.0;
    A.matrix[1][3] = 99.0;
    A.matrix[2][0] = -74.0;
    A.matrix[2][1] = 85.0;
    A.matrix[2][2] = 97.0;
    A.matrix[2][3] = 63.0;
    A.matrix[3][0] = -13.0;
    A.matrix[3][1] = 79.0;
    A.matrix[3][2] = -99.0;
    A.matrix[3][3] = -121.0;

    s21_calc_complements(&A, &Z);

    X.matrix[0][0] = -2879514.0;
    X.matrix[0][1] = -1236631.0;
    X.matrix[0][2] = -1685096.0;
    X.matrix[0][3] = 880697.0;
    X.matrix[1][0] = 1162090.0;
    X.matrix[1][1] = -714015.0;
    X.matrix[1][2] = 4046255.0;
    X.matrix[1][3] = -3901600.0;
    X.matrix[2][0] = 4362897.0;
    X.matrix[2][1] = -2049432.0;
    X.matrix[2][2] = -532912.0;
    X.matrix[2][3] = -1370781.0;
    X.matrix[3][0] = 3412773.0;
    X.matrix[3][1] = -1569493.0;
    X.matrix[3][2] = 3144517.0;
    X.matrix[3][3] = 1284666.0;

    res = s21_eq_matrix(&X, &Z);
    ck_assert_int_eq(res, SUCCESS);
  }
  s21_remove_matrix(&A);
  s21_remove_matrix(&X);
  s21_remove_matrix(&Z);
}
END_TEST

START_TEST(s21_calc_complements_02) {
  int res = 0;
  matrix_t A = {0};
  matrix_t Z = {0};
  matrix_t X = {0};

  int resA = s21_create_matrix(4, 4, &A);
  int resB = s21_create_matrix(4, 4, &X);
  if (resA == 0 && resB == 0) {
    A.matrix[0][0] = -1.0;
    A.matrix[0][1] = 2.0;
    A.matrix[0][2] = 7.0;
    A.matrix[0][3] = 9.0;
    A.matrix[1][0] = 1.0;
    A.matrix[1][1] = 0.0;
    A.matrix[1][2] = 0.0;
    A.matrix[1][3] = 0.0;
    A.matrix[2][0] = 47.0;
    A.matrix[2][1] = 13.0;
    A.matrix[2][2] = 17.0;
    A.matrix[2][3] = 21.0;
    A.matrix[3][0] = 22.0;
    A.matrix[3][1] = 7.0;
    A.matrix[3][2] = 1.0;
    A.matrix[3][3] = 3.0;

    s21_calc_complements(&A, &Z);

    X.matrix[0][0] = 0.0;
    X.matrix[0][1] = -30.0;
    X.matrix[0][2] = -108.0;
    X.matrix[0][3] = 106.0;
    X.matrix[1][0] = 138.0;
    X.matrix[1][1] = -726.0;
    X.matrix[1][2] = -1137.0;
    X.matrix[1][3] = 1061.0;
    X.matrix[2][0] = 0.0;
    X.matrix[2][1] = 12.0;
    X.matrix[2][2] = 57.0;
    X.matrix[2][3] = -47.0;
    X.matrix[3][0] = 0.0;
    X.matrix[3][1] = 6.0;
    X.matrix[3][2] = -75.0;
    X.matrix[3][3] = 57.0;

    res = s21_eq_matrix(&X, &Z);
    ck_assert_int_eq(res, SUCCESS);
  }
  s21_remove_matrix(&A);
  s21_remove_matrix(&X);
  s21_remove_matrix(&Z);
}
END_TEST

START_TEST(s21_calc_complements_03) {
  int res = 0;
  matrix_t A = {0};
  matrix_t Z = {0};

  int resA = s21_create_matrix(4, 3, &A);
  if (resA == 0) {
    A.matrix[0][0] = -1.0;
    A.matrix[0][1] = 2.0;
    A.matrix[0][2] = 7.0;
    A.matrix[1][0] = 1.0;
    A.matrix[1][1] = 0.0;
    A.matrix[1][2] = 0.0;
    A.matrix[2][0] = 47.0;
    A.matrix[2][1] = 13.0;
    A.matrix[2][2] = 17.0;
    A.matrix[3][0] = 22.0;
    A.matrix[3][1] = 7.0;
    A.matrix[3][2] = 1.0;

    res = s21_calc_complements(&A, &Z);
    ck_assert_int_eq(res, ERR_CAL);
  }
  s21_remove_matrix(&A);
  s21_remove_matrix(&Z);
}
END_TEST
START_TEST(s21_calc_complements_04) {
  int res = 0;
  matrix_t A = {0};
  matrix_t Z = {0};

  int resA = s21_create_matrix(2, 2, &A);
  s21_remove_matrix(&A);
  if (resA == 0) {
    res = s21_calc_complements(&A, &Z);
    ck_assert_int_eq(res, ERR_MAT);
  }
  s21_remove_matrix(&Z);
}
END_TEST
START_TEST(s21_inverse_matrix_01) {
  int res = 0;
  matrix_t A = {0};
  matrix_t Z = {0};
  matrix_t X = {0};

  int resA = s21_create_matrix(4, 4, &A);
  int resB = s21_create_matrix(4, 4, &X);
  if (resA == 0 && resB == 0) {
    A.matrix[0][0] = -1.0;
    A.matrix[0][1] = 2.0;
    A.matrix[0][2] = 7.0;
    A.matrix[0][3] = 9.0;
    A.matrix[1][0] = 1.0;
    A.matrix[1][1] = 0.0;
    A.matrix[1][2] = 0.0;
    A.matrix[1][3] = 0.0;
    A.matrix[2][0] = 47.0;
    A.matrix[2][1] = 13.0;
    A.matrix[2][2] = 17.0;
    A.matrix[2][3] = 21.0;
    A.matrix[3][0] = 22.0;
    A.matrix[3][1] = 7.0;
    A.matrix[3][2] = 1.0;
    A.matrix[3][3] = 3.0;

    s21_inverse_matrix(&A, &Z);

    X.matrix[0][0] = 0.0;
    X.matrix[0][1] = 1.0;
    X.matrix[0][2] = 0.0;
    X.matrix[0][3] = 0.0;
    X.matrix[1][0] = -5.0 / 23.0;
    X.matrix[1][1] = -121.0 / 23.0;
    X.matrix[1][2] = 2.0 / 23.0;
    X.matrix[1][3] = 1.0 / 23.0;
    X.matrix[2][0] = -18.0 / 23.0;
    X.matrix[2][1] = -379.0 / 46.0;
    X.matrix[2][2] = 19.0 / 46.0;
    X.matrix[2][3] = -25.0 / 46.0;
    X.matrix[3][0] = 53.0 / 69.0;
    X.matrix[3][1] = 1061.0 / 138.0;
    X.matrix[3][2] = -47.0 / 138.0;
    X.matrix[3][3] = 19.0 / 46.0;

    res = s21_eq_matrix(&X, &Z);
    ck_assert_int_eq(res, SUCCESS);
  }
  s21_remove_matrix(&A);
  s21_remove_matrix(&X);
  s21_remove_matrix(&Z);
}
END_TEST

START_TEST(s21_inverse_matrix_03) {
  int res = 0;
  matrix_t A = {0};
  matrix_t Z = {0};

  int resA = s21_create_matrix(3, 3, &A);
  if (resA == 0) {
    s21_init_matrix(1.0, &A);
    s21_remove_matrix(&A);

    res = s21_inverse_matrix(&A, &Z);
    ck_assert_int_eq(res, ERR_MAT);
  }
  s21_remove_matrix(&A);
  s21_remove_matrix(&Z);
}
END_TEST

START_TEST(s21_inverse_matrix_04) {
  int res = 0;
  matrix_t A = {0};
  matrix_t Z = {0};

  int resA = s21_create_matrix(3, 2, &A);
  if (resA == 0) {
    s21_init_matrix(1.0, &A);

    res = s21_inverse_matrix(&A, &Z);
    ck_assert_int_eq(res, ERR_CAL);
  }
  s21_remove_matrix(&A);
  s21_remove_matrix(&Z);
}
END_TEST

START_TEST(s21_inverse_matrix_05) {
  int res = 0;
  matrix_t A = {0};
  matrix_t Z = {0};

  int resA = s21_create_matrix(3, 3, &A);
  if (resA == 0) {
    s21_init_matrix(1.0, &A);
    s21_remove_matrix(&A);

    res = s21_inverse_matrix(&A, &Z);
    ck_assert_int_eq(res, ERR_MAT);
  }
  s21_remove_matrix(&Z);
  s21_remove_matrix(&A);
}
END_TEST

START_TEST(s21_inverse_matrix_06) {
  int res = 0;
  matrix_t A = {0};
  matrix_t Z = {0};
  matrix_t X = {0};

  int resA = s21_create_matrix(3, 3, &A);
  int resB = s21_create_matrix(3, 3, &X);
  if (resA == 0 && resB == 0) {
    A.matrix[0][0] = 1.0;
    A.matrix[0][1] = 2.0;
    A.matrix[0][2] = 3.0;
    A.matrix[1][0] = 4.0;
    A.matrix[1][1] = 5.0;
    A.matrix[1][2] = 6.0;
    A.matrix[2][0] = 8.0;
    A.matrix[2][1] = 6.0;
    A.matrix[2][2] = 9.0;

    s21_inverse_matrix(&A, &Z);

    X.matrix[0][0] = -3.0 / 5.0;
    X.matrix[0][1] = 0.0;
    X.matrix[0][2] = 1.0 / 5.0;
    X.matrix[1][0] = -4.0 / 5.0;
    X.matrix[1][1] = 1.0;
    X.matrix[1][2] = -2.0 / 5.0;
    X.matrix[2][0] = 16.0 / 15.0;
    X.matrix[2][1] = -2.0 / 3.0;
    X.matrix[2][2] = 1.0 / 5.0;

    res = s21_eq_matrix(&X, &Z);
    ck_assert_int_eq(res, SUCCESS);
  }
  s21_remove_matrix(&A);
  s21_remove_matrix(&X);
  s21_remove_matrix(&Z);
}
END_TEST

START_TEST(create_1) {
  int res = 0;
  const int rows = rand() % 100 + 1;
  const int cols = rand() % 100 + 1;
  matrix_t m = {0};
  res = s21_create_matrix(rows, cols, &m);
  if (res == 0) {
    for (int i = 0; i < rows; i++) {
      for (int j = 0; j < cols; j++) {
        m.matrix[i][j] = 0;
        ck_assert_ldouble_eq_tol(0, m.matrix[i][j], 1e-07);
      }
    }
    ck_assert_int_eq(m.rows, rows);
    ck_assert_int_eq(m.columns, cols);
    s21_remove_matrix(&m);
  }
}
END_TEST

START_TEST(create_2) {
  const int rows = 0;
  const int cols = 10;
  matrix_t m = {0};
  ck_assert_int_eq(s21_create_matrix(rows, cols, &m), ERR_MAT);
  s21_remove_matrix(&m);
}
END_TEST

START_TEST(create_3) {
  const int rows = 10;
  const int cols = 0;

  matrix_t m = {0};
  ck_assert_int_eq(s21_create_matrix(rows, cols, &m), ERR_MAT);
  s21_remove_matrix(&m);
}
END_TEST

START_TEST(transpose_matrix) {
  const int rows = rand() % 100 + 1;
  const int cols = rand() % 100 + 1;
  matrix_t m = {0};
  int resA = s21_create_matrix(rows, cols, &m);

  matrix_t check = {0};
  int resB = s21_create_matrix(cols, rows, &check);
  if (resA == 0 && resB == 0) {
    for (int i = 0; i < rows; i++) {
      for (int j = 0; j < cols; j++) {
        double rand_val = get_rand(-10e10, 10e10);
        m.matrix[i][j] = rand_val;
        check.matrix[j][i] = rand_val;
      }
    }

    matrix_t res = {0};
    ck_assert_int_eq(s21_transpose(&m, &res), OK);
    ck_assert_int_eq(s21_eq_matrix(&check, &res), SUCCESS);
    s21_remove_matrix(&res);
  }
  s21_remove_matrix(&m);
  s21_remove_matrix(&check);
}
END_TEST

START_TEST(transpose_matrix2) {
  int rows = rand() % 100 + 1;
  int cols = rand() % 100 + 1;
  rows = -rows;
  cols = -cols;
  matrix_t m = {0};
  int resA = s21_create_matrix(rows, cols, &m);
  matrix_t check = {0};
  int resB = s21_create_matrix(cols, rows, &check);
  if (resA == 0 && resB == 0) {
    matrix_t res = {0};
    ck_assert_int_eq(s21_transpose(&m, &res), ERR_MAT);
    s21_remove_matrix(&res);
  }
  s21_remove_matrix(&m);
  s21_remove_matrix(&check);
}
END_TEST

START_TEST(eq_matrix) {
  int resA = 0, resB = 0;
  const int rows = rand() % 100 + 1;
  const int cols = rand() % 100 + 1;
  matrix_t m = {0};
  resA = s21_create_matrix(rows, cols, &m);
  matrix_t mtx = {0};
  resB = s21_create_matrix(rows, cols, &mtx);
  if (resA == 0 && resB == 0) {
    for (int i = 0; i < rows; i++) {
      for (int j = 0; j < cols; j++) {
        double rand_val = get_rand(DBL_MIN, DBL_MAX);
        m.matrix[i][j] = rand_val;
        mtx.matrix[i][j] = rand_val;
      }
    }
    ck_assert_int_eq(s21_eq_matrix(&m, &mtx), SUCCESS);
  }
  s21_remove_matrix(&m);
  s21_remove_matrix(&mtx);
}
END_TEST

START_TEST(not_eq) {
  int resA = 0, resB = 0;
  matrix_t m = {0};
  const int rows = rand() % 100 + 1;
  const int cols = rand() % 100 + 1;
  resA = s21_create_matrix(rows, cols, &m);
  matrix_t mtx = {0};
  const int rows1 = rand() % 100 + 1;
  const int cols1 = rand() % 100 + 1;
  resB = s21_create_matrix(rows1, cols1, &mtx);
  if (resA == 0 && resB == 0) {
    for (int i = 0; i < rows; i++) {
      for (int j = 0; j < cols; j++) {
        m.matrix[i][j] = get_rand(DBL_MIN, DBL_MAX) + 1;
      }
    }
    for (int i = 0; i < rows1; i++) {
      for (int j = 0; j < cols1; j++) {
        mtx.matrix[i][j] = get_rand(DBL_MIN, DBL_MAX);
      }
    }
    ck_assert_int_eq(s21_eq_matrix(&m, &mtx), 0);
  }
  s21_remove_matrix(&mtx);
  s21_remove_matrix(&m);
}
END_TEST

START_TEST(not_eq1) {
  matrix_t m = {0};
  int rows = rand() % 100 + 1;
  int cols = rand() % 100 + 1;
  rows = -rows;
  cols = -cols;
  int resA = s21_create_matrix(rows, cols, &m);
  matrix_t mtx = {0};
  const int rows1 = rand() % 100 + 1;
  const int cols1 = rand() % 100 + 1;
  int resB = s21_create_matrix(rows1, cols1, &mtx);
  if (resA == 0 && resB == 0) {
    ck_assert_int_eq(s21_eq_matrix(&m, &mtx), FAILURE);
  }
  s21_remove_matrix(&mtx);
  s21_remove_matrix(&m);
}
END_TEST

START_TEST(zero_matrix) {
  matrix_t A = {0};
  matrix_t B = {0};
  int result = s21_eq_matrix(&A, &B);
  ck_assert_int_eq(0, result);
}

START_TEST(zero_matrix_1) {
  matrix_t A = {0};
  matrix_t B = {0};
  int resA = s21_create_matrix(0, 0, &A);
  int resB = s21_create_matrix(0, 0, &B);
  if (resA == 0 && resB == 0) {
    int result = s21_eq_matrix(&A, &B);
    ck_assert_int_eq(0, result);
  }
  s21_remove_matrix(&A);
  s21_remove_matrix(&B);
}

START_TEST(casual_matrix_1) {
  int resA = 0, resB = 0;
  matrix_t A = {0};
  matrix_t B = {0};
  resA = s21_create_matrix(1, 1, &A);
  resB = s21_create_matrix(1, 1, &B);
  if (resA == 0 && resB == 0) {
    A.matrix[0][0] = 1;
    B.matrix[0][0] = 1;
    int result = s21_eq_matrix(&A, &B);
    ck_assert_int_eq(1, result);
  }
  s21_remove_matrix(&B);
  s21_remove_matrix(&A);
}

START_TEST(casual_matrix_2) {
  int resA = 0, resB = 0;
  matrix_t A = {0};
  matrix_t B = {0};
  resA = s21_create_matrix(1, 1, &A);
  resB = s21_create_matrix(1, 1, &B);
  if (resA == 0 && resB == 0) {
    A.matrix[0][0] = 1;
    B.matrix[0][0] = 2;
    int result = s21_eq_matrix(&A, &B);
    ck_assert_int_eq(0, result);
  }
  s21_remove_matrix(&B);
  s21_remove_matrix(&A);
}

START_TEST(casual_matrix_3) {
  int resA = 0, resB = 0;
  matrix_t A = {0};
  matrix_t B = {0};
  resA = s21_create_matrix(2, 2, &A);
  resB = s21_create_matrix(2, 2, &B);
  if (resA == 0 && resB == 0) {
    A.matrix[0][0] = 1;
    A.matrix[0][1] = 2;
    A.matrix[1][0] = 3;
    A.matrix[1][1] = 4;
    B.matrix[0][0] = 1;
    B.matrix[0][1] = 2;
    B.matrix[1][0] = 3;
    B.matrix[1][1] = 4;
    int result = s21_eq_matrix(&A, &B);
    ck_assert_int_eq(1, result);
  }
  s21_remove_matrix(&B);
  s21_remove_matrix(&A);
}

START_TEST(casual_matrix_4) {
  matrix_t A = {0};
  matrix_t B = {0};
  int resA = s21_create_matrix(2, 2, &A);
  int resB = s21_create_matrix(2, 2, &B);
  if (resA == 0 && resB == 0) {
    A.matrix[0][0] = 1;
    A.matrix[0][1] = 2;
    A.matrix[1][0] = 3;
    A.matrix[1][1] = 4;
    B.matrix[0][0] = 1;
    B.matrix[0][1] = 2;
    B.matrix[1][0] = 3;
    B.matrix[1][1] = 4;
    int result = s21_eq_matrix(&A, &B);
    ck_assert_int_eq(1, result);
  }
  s21_remove_matrix(&B);
  s21_remove_matrix(&A);
}

START_TEST(casual_matrix_5) {
  matrix_t A = {0};
  matrix_t B = {0};
  int resA = s21_create_matrix(2, 2, &A);
  int resB = s21_create_matrix(2, 2, &B);
  if (resA == 0 && resB == 0) {
    A.matrix[0][0] = 1.01;
    A.matrix[0][1] = 2;
    A.matrix[1][0] = 3.05;
    A.matrix[1][1] = 4;
    B.matrix[0][0] = 1.01;
    B.matrix[0][1] = 2;
    B.matrix[1][0] = 3.05;
    B.matrix[1][1] = 4;
    int result = s21_eq_matrix(&A, &B);
    ck_assert_int_eq(1, result);
  }
  s21_remove_matrix(&B);
  s21_remove_matrix(&A);
}

START_TEST(casual_matrix_6) {
  matrix_t A = {0};
  matrix_t B = {0};
  int resA = s21_create_matrix(2, 2, &A);
  int resB = s21_create_matrix(2, 2, &B);
  if (resA == 0 && resB == 0) {
    A.matrix[0][0] = 1.01;
    A.matrix[0][1] = -2;
    A.matrix[1][0] = 3.05;
    A.matrix[1][1] = -4;
    B.matrix[0][0] = 1.01;
    B.matrix[0][1] = -2;
    B.matrix[1][0] = 3.05;
    B.matrix[1][1] = -4;
    int result = s21_eq_matrix(&A, &B);
    ck_assert_int_eq(1, result);
  }
  s21_remove_matrix(&B);
  s21_remove_matrix(&A);
}

START_TEST(casual_matrix_7) {
  matrix_t A = {0};
  matrix_t B = {0};
  int resA = s21_create_matrix(2, 2, &A);
  int resB = s21_create_matrix(2, 2, &B);
  if (resA == 0 && resB == 0) {
    A.matrix[0][0] = 1.00000000234;
    A.matrix[0][1] = 2;
    A.matrix[1][0] = 3.05;
    A.matrix[1][1] = 4;
    B.matrix[0][0] = 1;
    B.matrix[0][1] = 2;
    B.matrix[1][0] = 3.05;
    B.matrix[1][1] = 4;
    int result = s21_eq_matrix(&A, &B);
    ck_assert_int_eq(1, result);
  }
  s21_remove_matrix(&B);
  s21_remove_matrix(&A);
}

START_TEST(casual_matrix_8) {
  matrix_t A = {0};
  matrix_t B = {0};
  int resA = s21_create_matrix(2, 2, &A);
  int resB = s21_create_matrix(2, 2, &B);
  if (resA == 0 && resB == 0) {
    A.matrix[0][0] = 1.0001;
    A.matrix[0][1] = 2;
    A.matrix[1][0] = 3.05;
    A.matrix[1][1] = 4;
    B.matrix[0][0] = 1;
    B.matrix[0][1] = 2;
    B.matrix[1][0] = 3.05;
    B.matrix[1][1] = 4;
    int result = s21_eq_matrix(&A, &B);
    ck_assert_int_eq(0, result);
  }
  s21_remove_matrix(&B);
  s21_remove_matrix(&A);
}

START_TEST(sum_matrix) {
  int rows = rand() % 100 + 1;
  int cols = rand() % 100 + 1;
  matrix_t m = {0};
  int resA = s21_create_matrix(rows, cols, &m);
  matrix_t mtx = {0};
  int resB = s21_create_matrix(rows, cols, &mtx);
  matrix_t check = {0};
  int resC = s21_create_matrix(rows, cols, &check);
  if (resA == 0 && resB == 0 && resC == 0) {
    for (int i = 0; i < rows; i++) {
      for (int j = 0; j < cols; j++) {
        m.matrix[i][j] = get_rand(DBL_MIN, DBL_MAX);
        mtx.matrix[i][j] = get_rand(DBL_MIN, DBL_MAX);
        check.matrix[i][j] = m.matrix[i][j] + mtx.matrix[i][j];
      }
    }
    matrix_t res = {0};

    ck_assert_int_eq(s21_sum_matrix(&m, &mtx, &res), OK);
    ck_assert_int_eq(s21_eq_matrix(&check, &res), SUCCESS);
    s21_remove_matrix(&res);
  }
  s21_remove_matrix(&m);
  s21_remove_matrix(&mtx);
  s21_remove_matrix(&check);
}
END_TEST

START_TEST(sum_matrix1) {
  matrix_t m = {0};
  const int rows = rand() % 100 + 1;
  const int cols = rand() % 100 + 1;
  int resA = s21_create_matrix(rows, cols, &m);
  matrix_t mtx = {0};
  const int rows1 = rand() % 100 + 1;
  const int cols1 = rand() % 100 + 1;
  int resB = s21_create_matrix(rows1, cols1, &mtx);
  if (resA == 0 && resB == 0) {
    for (int i = 0; i < rows; i++) {
      for (int j = 0; j < cols; j++) {
        m.matrix[i][j] = get_rand(DBL_MIN, DBL_MAX) + 1;
      }
    }
    for (int i = 0; i < rows1; i++) {
      for (int j = 0; j < cols1; j++) {
        mtx.matrix[i][j] = get_rand(DBL_MIN, DBL_MAX);
      }
    }

    matrix_t res = {0};
    ck_assert_int_eq(s21_sum_matrix(&m, &mtx, &res), ERR_CAL);
    s21_remove_matrix(&res);
  }
  s21_remove_matrix(&m);
  s21_remove_matrix(&mtx);
}
END_TEST

START_TEST(sum_matrix2) {
  int rows = rand() % 100 + 1;
  int cols = rand() % 100 + 1;
  rows = -rows;
  cols = -cols;
  matrix_t m = {0};
  int resA = s21_create_matrix(rows, cols, &m);
  matrix_t mtx = {0};
  int resB = s21_create_matrix(rows, cols, &mtx);
  if (resA == 0 && resB == 0) {
    matrix_t res = {0};
    ck_assert_int_eq(s21_sum_matrix(&m, &mtx, &res), ERR_MAT);
    s21_remove_matrix(&res);
  }
  s21_remove_matrix(&m);
  s21_remove_matrix(&mtx);
}
END_TEST

START_TEST(sub_matrix) {
  int rows = rand() % 100 + 1;
  int cols = rand() % 100 + 1;
  matrix_t m = {0};
  int resA = s21_create_matrix(rows, cols, &m);
  matrix_t mtx = {0};
  int resB = s21_create_matrix(rows, cols, &mtx);
  matrix_t check = {0};
  int resC = s21_create_matrix(rows, cols, &check);
  if (resA == 0 && resB == 0 && resC == 0) {
    for (int i = 0; i < rows; i++) {
      for (int j = 0; j < cols; j++) {
        m.matrix[i][j] = get_rand(DBL_MIN, DBL_MAX);
        mtx.matrix[i][j] = get_rand(DBL_MIN, DBL_MAX);
        check.matrix[i][j] = m.matrix[i][j] - mtx.matrix[i][j];
      }
    }
    matrix_t res = {0};
    ck_assert_int_eq(s21_sub_matrix(&m, &mtx, &res), OK);
    ck_assert_int_eq(s21_eq_matrix(&check, &res), SUCCESS);

    s21_remove_matrix(&m);
    s21_remove_matrix(&mtx);
    s21_remove_matrix(&res);
    s21_remove_matrix(&check);
  }
}
END_TEST

START_TEST(sub_matrix1) {
  matrix_t m = {0};
  const int rows = rand() % 100 + 1;
  const int cols = rand() % 100 + 1;
  int resA = s21_create_matrix(rows, cols, &m);
  matrix_t mtx = {0};
  const int rows1 = rand() % 100 + 1;
  const int cols1 = rand() % 100 + 1;
  int resB = s21_create_matrix(rows1, cols1, &mtx);
  if (resA == 0 && resB == 0) {
    for (int i = 0; i < rows; i++) {
      for (int j = 0; j < cols; j++) {
        m.matrix[i][j] = get_rand(DBL_MIN, DBL_MAX) + 1;
      }
    }
    for (int i = 0; i < rows1; i++) {
      for (int j = 0; j < cols1; j++) {
        mtx.matrix[i][j] = get_rand(DBL_MIN, DBL_MAX);
      }
    }

    matrix_t res = {0};
    ck_assert_int_eq(s21_sub_matrix(&m, &mtx, &res), ERR_CAL);
    s21_remove_matrix(&m);
    s21_remove_matrix(&mtx);
    s21_remove_matrix(&res);
  }
}
END_TEST

START_TEST(sub_matrix2) {
  int rows = rand() % 100 + 1;
  int cols = rand() % 100 + 1;
  rows = -rows;
  cols = -cols;
  matrix_t m = {0};
  int resA = s21_create_matrix(rows, cols, &m);
  matrix_t mtx = {0};
  int resB = s21_create_matrix(rows, cols, &mtx);
  if (resA == 0 && resB == 0) {
    matrix_t res = {0};
    ck_assert_int_eq(s21_sub_matrix(&m, &mtx, &res), ERR_MAT);

    s21_remove_matrix(&m);
    s21_remove_matrix(&mtx);
    s21_remove_matrix(&res);
  }
}
END_TEST

START_TEST(simple_mult) {
  const int rows = 2;
  const int cols = 3;
  matrix_t m = {0};
  int resA = s21_create_matrix(rows, cols, &m);
  matrix_t mtx = {0};
  int resB = s21_create_matrix(cols, rows, &mtx);
  matrix_t check = {0};
  int resC = s21_create_matrix(m.rows, mtx.columns, &check);

  if (resA == 0 && resB == 0 && resC == 0) {
    for (int i = 0, c = 1; i < rows; i++)
      for (int j = 0; j < cols; j++) m.matrix[i][j] = c++;

    for (int i = 0, c = 7; i < cols; i++)
      for (int j = 0; j < rows; j++) mtx.matrix[i][j] = c++;

    check.matrix[0][0] = 58;
    check.matrix[0][1] = 64;
    check.matrix[1][0] = 139;
    check.matrix[1][1] = 154;

    matrix_t res = {0};
    ck_assert_int_eq(s21_mult_matrix(&m, &mtx, &res), OK);
    ck_assert_int_eq(s21_eq_matrix(&check, &res), SUCCESS);
    s21_remove_matrix(&res);
  }
  s21_remove_matrix(&m);
  s21_remove_matrix(&mtx);
  s21_remove_matrix(&check);
}
END_TEST

START_TEST(mult_matrix2) {
  int rows = rand() % 100 + 1;
  int cols = rand() % 100 + 1;
  rows = -rows;
  cols = -cols;
  matrix_t m = {0};
  int resA = s21_create_matrix(rows, cols, &m);
  matrix_t mtx = {0};
  int resB = s21_create_matrix(cols, rows, &mtx);
  matrix_t check = {0};
  int resC = s21_create_matrix(m.rows, mtx.columns, &check);
  if (resA == 0 && resB == 0 && resC == 0) {
    matrix_t res = {0};
    ck_assert_int_eq(s21_mult_matrix(&m, &mtx, &res), ERR_MAT);
    s21_remove_matrix(&res);
  }
  s21_remove_matrix(&m);
  s21_remove_matrix(&mtx);
  s21_remove_matrix(&check);
}
END_TEST

START_TEST(mult_matrix3) {
  matrix_t m = {0};
  int rows = 2;
  int cols = 3;
  int resA = s21_create_matrix(rows, cols, &m);
  matrix_t mtx = {0};
  int rows1 = 4;
  int cols1 = 5;
  int resB = s21_create_matrix(rows1, cols1, &mtx);

  matrix_t check = {0};
  int resC = s21_create_matrix(m.rows, mtx.columns, &check);
  if (resA == 0 && resB == 0 && resC == 0) {
    matrix_t res = {0};
    ck_assert_int_eq(s21_mult_matrix(&m, &mtx, &res), ERR_CAL);
    s21_remove_matrix(&res);
  }
  s21_remove_matrix(&m);
  s21_remove_matrix(&mtx);
  s21_remove_matrix(&check);
}
END_TEST

START_TEST(mult_number_matrix) {
  int rows = rand() % 100 + 1;
  int cols = rand() % 100 + 1;
  matrix_t m = {0};
  int resA = s21_create_matrix(rows, cols, &m);
  matrix_t check = {0};
  int resB = s21_create_matrix(rows, cols, &check);
  if (resA == 0 && resB == 0) {
    double mult_number = get_rand(-10e5, 10e5);
    for (int i = 0; i < rows; i++) {
      for (int j = 0; j < cols; j++) {
        m.matrix[i][j] = get_rand(DBL_MIN, DBL_MAX);
        check.matrix[i][j] = m.matrix[i][j] * mult_number;
      }
    }
    matrix_t res = {0};
    ck_assert_int_eq(s21_mult_number(&m, mult_number, &res), OK);
    ck_assert_int_eq(s21_eq_matrix(&check, &res), SUCCESS);
    s21_remove_matrix(&res);
  }
  s21_remove_matrix(&m);
  s21_remove_matrix(&check);
}
END_TEST

START_TEST(mult_number_matrix2) {
  int rows = rand() % 100 + 1;
  int cols = rand() % 100 + 1;
  rows = -rows;
  cols = -cols;
  matrix_t m = {0};
  int resA = s21_create_matrix(rows, cols, &m);
  matrix_t check = {0};
  int resB = s21_create_matrix(rows, cols, &check);
  if (resA == 0 && resB == 0) {
    double mult_number = get_rand(-10e5, 10e5);

    matrix_t res = {0};
    ck_assert_int_eq(s21_mult_number(&m, mult_number, &res), ERR_MAT);
    s21_remove_matrix(&res);
  }
  s21_remove_matrix(&m);
  s21_remove_matrix(&check);
}
END_TEST

START_TEST(test_one_by_one) {
  matrix_t m = {0};
  matrix_t result = {0};
  int codec = s21_create_matrix(1, 1, &m);
  if (codec) {
    int code = s21_calc_complements(&m, &result);
    ck_assert_int_eq(code, ERR_CAL);
  }
  s21_remove_matrix(&m);
  s21_remove_matrix(&result);
}
END_TEST

START_TEST(test_incorrect) {
  matrix_t m = {0};
  matrix_t result = {0};
  int code = s21_calc_complements(&m, &result);
  ck_assert_int_eq(code, ERR_MAT);
}
END_TEST

START_TEST(test_not_sqare) {
  matrix_t m = {0};
  matrix_t result = {0};
  int codec = s21_create_matrix(3, 4, &m);
  if (codec == 0) {
    int code = s21_calc_complements(&m, &result);
    ck_assert_int_eq(code, ERR_CAL);
  }
  s21_remove_matrix(&m);
  s21_remove_matrix(&result);
}
END_TEST

START_TEST(determinant1) {
  const int size = 5;
  matrix_t m = {0};
  int resA = s21_create_matrix(size, size, &m);
  if (resA == 0) {
    for (int i = 0; i < size; i++)
      for (int j = 0; j < size; j++) m.matrix[i][j] = j;
    double res = 0;
    int code = s21_determinant(&m, &res);
    ck_assert_double_eq_tol(res, 0, 1e-6);
    ck_assert_int_eq(code, OK);
  }
  s21_remove_matrix(&m);
}
END_TEST

START_TEST(determinant2) {
  const int size = 4;
  matrix_t m = {0};
  int resA = s21_create_matrix(size, size, &m);
  if (resA == 0) {
    for (int i = 0; i < size; i++)
      for (int j = 0; j < size; j++) m.matrix[i][j] = j + i;

    double res = 0;
    int code = s21_determinant(&m, &res);
    ck_assert_double_eq_tol(res, 0, 1e-6);
    ck_assert_int_eq(code, OK);
  }
  s21_remove_matrix(&m);
}
END_TEST

START_TEST(determinant3) {
  const int size = 5;
  matrix_t m = {0};
  int resA = s21_create_matrix(size, size, &m);
  if (resA == 0) {
    m.matrix[0][1] = 6;
    m.matrix[0][2] = -2;
    m.matrix[0][3] = -1;
    m.matrix[0][4] = 5;
    m.matrix[1][3] = -9;
    m.matrix[1][4] = -7;
    m.matrix[2][1] = 15;
    m.matrix[2][2] = 35;
    m.matrix[3][1] = -1;
    m.matrix[3][2] = -11;
    m.matrix[3][3] = -2;
    m.matrix[3][4] = 1;
    m.matrix[4][0] = -2;
    m.matrix[4][1] = -2;
    m.matrix[4][2] = 3;
    m.matrix[4][4] = -2;

    double res = 0;
    int code = s21_determinant(&m, &res);
    ck_assert_double_eq_tol(res, 2480, 1e-6);
    ck_assert_int_eq(code, OK);
  }
  s21_remove_matrix(&m);
}
END_TEST

START_TEST(determinant4) {
  const int size = 3;
  matrix_t m = {0};
  int resA = s21_create_matrix(size, size, &m);
  if (resA == 0) {
    m.matrix[0][0] = 2;
    m.matrix[0][1] = 3;
    m.matrix[0][2] = 1;
    m.matrix[1][0] = 7;
    m.matrix[1][1] = 4;
    m.matrix[1][2] = 1;
    m.matrix[2][0] = 9;
    m.matrix[2][1] = -2;
    m.matrix[2][2] = 1;

    double res = 0;
    int code = s21_determinant(&m, &res);
    ck_assert_double_eq_tol(res, -32, 1e-6);
    ck_assert_int_eq(code, OK);
  }
  s21_remove_matrix(&m);
}
END_TEST

START_TEST(determinant5) {
  const int size = 2;
  matrix_t m = {0};
  int resA = s21_create_matrix(size, size, &m);
  if (resA == 0) {
    m.matrix[0][0] = -5;
    m.matrix[0][1] = -4;
    m.matrix[1][0] = -2;
    m.matrix[1][1] = -3;

    double res = 0;
    int code = s21_determinant(&m, &res);
    ck_assert_double_eq_tol(res, 7, 1e-6);
    ck_assert_int_eq(code, OK);
  }
  s21_remove_matrix(&m);
}
END_TEST

START_TEST(determinant6) {
  const int size = 1;
  matrix_t m = {0};
  int resA = s21_create_matrix(size, size, &m);
  if (resA == 0) {
    m.matrix[0][0] = -5;

    double res = 0;
    int code = s21_determinant(&m, &res);
    ck_assert_double_eq_tol(res, -5, 1e-6);
    ck_assert_int_eq(code, OK);
  }
  s21_remove_matrix(&m);
}
END_TEST

START_TEST(determinant7) {
  matrix_t m = {0};
  int rows = rand() % 100 + 1;
  rows = -rows;
  int resA = s21_create_matrix(rows, rows, &m);
  if (resA == 0) {
    double res = 0;
    int code = s21_determinant(&m, &res);
    ck_assert_int_eq(code, ERR_MAT);
  }
  s21_remove_matrix(&m);
}
END_TEST

START_TEST(determinant8) {
  matrix_t m = {0};
  int rows = 4;
  int cols = 5;
  int resA = s21_create_matrix(rows, cols, &m);
  if (resA == 0) {
    for (int i = 0; i < rows; i++) {
      for (int j = 0; j < cols; j++) {
        m.matrix[i][j] = get_rand(DBL_MIN, DBL_MAX) + 1;
      }
    }
    double res = 0;
    int code = s21_determinant(&m, &res);
    ck_assert_int_eq(code, ERR_CAL);
  }
  s21_remove_matrix(&m);
}
END_TEST

START_TEST(s21_inverse_1) {
  matrix_t A, C;
  int resA = s21_create_matrix(3, 3, &A);
  int resB = s21_create_matrix(3, 3, &C);
  if (resA == 0 && resB == 0) {
    C.matrix[0][0] = 1.0;
    C.matrix[0][1] = -1.0;
    C.matrix[0][2] = 1.0;
    C.matrix[1][0] = -38.0;
    C.matrix[1][1] = 41.0;
    C.matrix[1][2] = -34.0;
    C.matrix[2][0] = 27.0;
    C.matrix[2][1] = -29.0;
    C.matrix[2][2] = 24.0;
    A.matrix[0][0] = 2.0;
    A.matrix[0][1] = 5.0;
    A.matrix[0][2] = 7.0;
    A.matrix[1][0] = 6.0;
    A.matrix[1][1] = 3.0;
    A.matrix[1][2] = 4.0;
    A.matrix[2][0] = 5.0;
    A.matrix[2][1] = -2.0;
    A.matrix[2][2] = -3.0;
    matrix_t B;
    s21_inverse_matrix(&A, &B);
    int res = s21_eq_matrix(&B, &C);
    ck_assert_int_eq(res, 1);
    s21_remove_matrix(&B);
  }
  s21_remove_matrix(&A);
  s21_remove_matrix(&C);
}
END_TEST

START_TEST(determinant) {
  const int size = 2;
  matrix_t m = {0};
  int resA = s21_create_matrix(size, size, &m);
  if (resA == 0) {
    m.matrix[0][0] = 1;
    m.matrix[0][1] = 1;
    m.matrix[1][0] = 1;
    m.matrix[1][1] = 1;

    matrix_t result = {0};
    int code = s21_inverse_matrix(&m, &result);
    ck_assert_int_eq(code, ERR_CAL);
    s21_remove_matrix(&result);
  }
  s21_remove_matrix(&m);
}
END_TEST

START_TEST(test_normal) {
  matrix_t m = {0};
  matrix_t expected = {0};
  int codec1, codec2;
  codec1 = s21_create_matrix(3, 3, &m);
  codec2 = s21_create_matrix(3, 3, &expected);

  if (codec1 == 0 && codec2 == 0) {
    m.matrix[0][0] = 2;
    m.matrix[0][1] = 5;
    m.matrix[0][2] = 7;

    m.matrix[1][0] = 6;
    m.matrix[1][1] = 3;
    m.matrix[1][2] = 4;

    m.matrix[2][0] = 5;
    m.matrix[2][1] = -2;
    m.matrix[2][2] = -3;

    expected.matrix[0][0] = 1;
    expected.matrix[0][1] = -1;
    expected.matrix[0][2] = 1;

    expected.matrix[1][0] = -38;
    expected.matrix[1][1] = 41;
    expected.matrix[1][2] = -34;

    expected.matrix[2][0] = 27;
    expected.matrix[2][1] = -29;
    expected.matrix[2][2] = 24;
    matrix_t result = {0};
    int code = s21_inverse_matrix(&m, &result);

    ck_assert_int_eq(s21_eq_matrix(&result, &expected), SUCCESS);
    ck_assert_int_eq(code, OK);

    s21_remove_matrix(&result);
  }
  s21_remove_matrix(&m);
  s21_remove_matrix(&expected);
}
END_TEST

START_TEST(inverse) {
  const int size = 3;
  matrix_t m = {0};
  matrix_t expected = {0};
  int resA = s21_create_matrix(size, size, &m);
  int resB = s21_create_matrix(size, size, &expected);
  if (resA == 0 && resB == 0) {
    m.matrix[0][0] = 2;
    m.matrix[0][1] = 5;
    m.matrix[0][2] = 7;
    m.matrix[1][0] = 6;
    m.matrix[1][1] = 3;
    m.matrix[1][2] = 4;
    m.matrix[2][0] = 5;
    m.matrix[2][1] = -2;
    m.matrix[2][2] = -3;

    matrix_t res = {0};
    s21_inverse_matrix(&m, &res);

    expected.matrix[0][0] = 1;
    expected.matrix[0][1] = -1;
    expected.matrix[0][2] = 1;
    expected.matrix[1][0] = -38;
    expected.matrix[1][1] = 41;
    expected.matrix[1][2] = -34;
    expected.matrix[2][0] = 27;
    expected.matrix[2][1] = -29;
    expected.matrix[2][2] = 24;

    ck_assert_int_eq(s21_eq_matrix(&expected, &res), SUCCESS);
    s21_remove_matrix(&res);
  }
  s21_remove_matrix(&expected);
  s21_remove_matrix(&m);
}
END_TEST

Suite *suite_inverse_matrix(void) {
  Suite *s = suite_create("suite_inverse_matrix");
  TCase *tc = tcase_create("case_inverse_matrix");

  tcase_add_test(tc, inverse);
  tcase_add_test(tc, s21_inverse_1);
  tcase_add_test(tc, determinant);

  tcase_add_test(tc, test_normal);
  tcase_add_test(tc, test_not_sqare);
  tcase_add_test(tc, test_incorrect);
  tcase_add_test(tc, test_one_by_one);

  suite_add_tcase(s, tc);
  return s;
}

Suite *suite_determinant(void) {
  Suite *s = suite_create("suite_determinant");
  TCase *tc = tcase_create("case_determinant");

  tcase_add_test(tc, determinant1);
  tcase_add_test(tc, determinant2);
  tcase_add_test(tc, determinant3);
  tcase_add_test(tc, determinant4);
  tcase_add_test(tc, determinant5);
  tcase_add_test(tc, determinant6);
  tcase_add_loop_test(tc, determinant7, 0, 100);
  tcase_add_test(tc, determinant8);

  suite_add_tcase(s, tc);
  return s;
}

Suite *suite_calc_complements(void) {
  Suite *s = suite_create("suite_calc_complements");
  TCase *tc = tcase_create("case_calc_complements");

  tcase_add_test(tc, test_normal);
  tcase_add_test(tc, test_not_sqare);
  tcase_add_test(tc, test_incorrect);
  tcase_add_test(tc, test_one_by_one);

  suite_add_tcase(s, tc);
  return s;
}

Suite *suite_mult_number_matrix(void) {
  Suite *s = suite_create("suite_mult_number_matrix");
  TCase *tc = tcase_create("case_mult_number_matrix");

  tcase_add_loop_test(tc, mult_number_matrix, 0, 100);
  tcase_add_loop_test(tc, mult_number_matrix2, 0, 100);

  suite_add_tcase(s, tc);
  return s;
}

Suite *suite_mult_matrix(void) {
  Suite *s = suite_create("suite_mult_matrix");
  TCase *tc = tcase_create("case_mult_matrix");

  tcase_add_loop_test(tc, mult_matrix2, 0, 100);
  tcase_add_test(tc, mult_matrix3);
  tcase_add_test(tc, simple_mult);

  suite_add_tcase(s, tc);
  return s;
}

Suite *suite_sub_matrix(void) {
  Suite *s = suite_create("suite_sub_matrix");
  TCase *tc = tcase_create("case_sub_matrix");

  tcase_add_loop_test(tc, sub_matrix, 0, 100);
  tcase_add_loop_test(tc, sub_matrix1, 0, 100);
  tcase_add_loop_test(tc, sub_matrix2, 0, 100);

  suite_add_tcase(s, tc);
  return s;
}

Suite *suite_sum_matrix(void) {
  Suite *s = suite_create("suite_sum_matrix");
  TCase *tc = tcase_create("case_sum_matrix");

  tcase_add_loop_test(tc, sum_matrix, 0, 100);
  tcase_add_loop_test(tc, sum_matrix1, 0, 100);
  tcase_add_loop_test(tc, sum_matrix2, 0, 100);

  suite_add_tcase(s, tc);
  return s;
}

Suite *suite_eq_matrix(void) {
  Suite *s = suite_create("suite_eq_matrix");
  TCase *tc = tcase_create("case_eq_matrix");

  tcase_add_test(tc, not_eq);
  tcase_add_loop_test(tc, eq_matrix, 0, 100);
  tcase_add_test(tc, not_eq1);
  tcase_add_test(tc, zero_matrix);
  tcase_add_test(tc, zero_matrix_1);
  tcase_add_test(tc, casual_matrix_1);
  tcase_add_test(tc, casual_matrix_2);
  tcase_add_test(tc, casual_matrix_3);
  tcase_add_test(tc, casual_matrix_4);
  tcase_add_test(tc, casual_matrix_5);
  tcase_add_test(tc, casual_matrix_6);
  tcase_add_test(tc, casual_matrix_7);
  tcase_add_test(tc, casual_matrix_8);

  suite_add_tcase(s, tc);
  return s;
}

Suite *suite_transpose_matrix(void) {
  Suite *s = suite_create("suite_transpose_matrix");
  TCase *tc = tcase_create("case_transpose_matrix");

  tcase_add_loop_test(tc, transpose_matrix, 0, 100);
  tcase_add_loop_test(tc, transpose_matrix2, 0, 100);

  suite_add_tcase(s, tc);
  return s;
}

Suite *suite_create_matrix(void) {
  Suite *s = suite_create("suite_create_matrix");
  TCase *tc = tcase_create("case_create_matrix");

  tcase_add_test(tc, create_1);
  tcase_add_test(tc, create_2);
  tcase_add_test(tc, create_3);

  suite_add_tcase(s, tc);
  return s;
}

Suite *suite_matrix_gather(void) {
  Suite *s = suite_create("suite_matrix_gather");
  TCase *tc = tcase_create("suite_matrix_gather");
  tcase_add_test(tc, s21_create_matrix_01);
  tcase_add_test(tc, s21_create_matrix_02);
  tcase_add_test(tc, s21_create_matrix_03);
  tcase_add_test(tc, s21_create_matrix_04);
  tcase_add_test(tc, s21_create_matrix_05);
  tcase_add_test(tc, s21_create_matrix_06);
  tcase_add_test(tc, s21_create_matrix_07);
  tcase_add_test(tc, s21_create_matrix_08);
  tcase_add_test(tc, s21_create_matrix_09);

  tcase_add_test(tc, s21_remove_matrix_01);
  tcase_add_test(tc, s21_remove_matrix_02);
  tcase_add_test(tc, s21_remove_matrix_03);
  tcase_add_test(tc, s21_remove_matrix_04);
  tcase_add_test(tc, s21_remove_matrix_05);

  tcase_add_test(tc, s21_eq_matrix_01);
  tcase_add_test(tc, s21_eq_matrix_06);

  tcase_add_test(tc, s21_sum_matrix_01);
  tcase_add_test(tc, s21_sum_matrix_02);
  tcase_add_test(tc, s21_sum_matrix_03);
  tcase_add_test(tc, s21_sum_matrix_04);
  tcase_add_test(tc, s21_sum_matrix_05);
  tcase_add_test(tc, s21_sum_matrix_06);
  tcase_add_test(tc, s21_sum_matrix_07);
  tcase_add_test(tc, s21_sum_matrix_08);

  tcase_add_test(tc, s21_sub_matrix_01);
  tcase_add_test(tc, s21_sub_matrix_02);
  tcase_add_test(tc, s21_sub_matrix_03);
  tcase_add_test(tc, s21_sub_matrix_04);
  tcase_add_test(tc, s21_sub_matrix_05);
  tcase_add_test(tc, s21_sub_matrix_06);
  tcase_add_test(tc, s21_sub_matrix_07);
  tcase_add_test(tc, s21_sub_matrix_08);

  tcase_add_test(tc, s21_mult_number_01);
  tcase_add_test(tc, s21_mult_number_02);
  tcase_add_test(tc, s21_mult_number_03);
  tcase_add_test(tc, s21_mult_number_04);
  tcase_add_test(tc, s21_mult_number_05);
  tcase_add_test(tc, s21_mult_number_06);

  tcase_add_test(tc, s21_mult_matrix_01);
  tcase_add_test(tc, s21_mult_matrix_02);
  tcase_add_test(tc, s21_mult_matrix_03);
  tcase_add_test(tc, s21_mult_matrix_04);
  tcase_add_test(tc, s21_mult_matrix_05);
  tcase_add_test(tc, s21_mult_matrix_06);
  tcase_add_test(tc, s21_mult_matrix_07);

  tcase_add_test(tc, s21_transpose_01);
  tcase_add_test(tc, s21_transpose_03);

  tcase_add_test(tc, s21_determinant_01);
  tcase_add_test(tc, s21_determinant_02);
  tcase_add_test(tc, s21_determinant_03);
  tcase_add_test(tc, s21_determinant_04);
  tcase_add_test(tc, s21_determinant_05);

  tcase_add_test(tc, s21_calc_complements_01);
  tcase_add_test(tc, s21_calc_complements_02);
  tcase_add_test(tc, s21_calc_complements_03);
  tcase_add_test(tc, s21_calc_complements_04);

  tcase_add_test(tc, s21_inverse_matrix_01);
  tcase_add_test(tc, s21_inverse_matrix_03);
  tcase_add_test(tc, s21_inverse_matrix_04);
  tcase_add_test(tc, s21_inverse_matrix_05);
  tcase_add_test(tc, s21_inverse_matrix_06);
  suite_add_tcase(s, tc);
  return s;
}

void testcases(Suite *testcase) {
  static int counter_testcase = 1;

  if (counter_testcase > 0) putchar('\n');
  counter_testcase++;
  SRunner *sr = srunner_create(testcase);

  srunner_set_fork_status(sr, CK_NOFORK);
  srunner_run_all(sr, CK_VERBOSE);

  srunner_free(sr);
}

double get_rand(double min, double max) {
  double val = (double)rand() / RAND_MAX;
  return min + val * (max - min);
}

int main(void) {
  Suite *list_cases[] = {suite_create_matrix(),    suite_eq_matrix(),
                         suite_sub_matrix(),       suite_sum_matrix(),
                         suite_mult_matrix(),      suite_mult_number_matrix(),
                         suite_transpose_matrix(), suite_calc_complements(),
                         suite_determinant(),      suite_inverse_matrix(),
                         suite_matrix_gather(),    NULL};
  for (Suite **current_testcase = list_cases; *current_testcase != NULL;
       current_testcase++) {
    testcases(*current_testcase);
  }
  return 0;
}
